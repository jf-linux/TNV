...
service_c::service_c(acl::socket_stream* conn, acl::session* sess):
	HttpServlet(conn, sess) {  // 交给基类部分
}

bool service_c::doGet(acl::HttpServletRequest& req,
	acl::HttpServletResponse& res) {
	// 从请求中提取资源路径
	acl::string path = req.getPathInfo();  //获取路劲信息(基类已经帮你解析好了)

       // 将文件路径的前n个字符和ROUTE_FILES比较
	...
	// 发送响应
	return res.write(NULL, 0); // 不需要额外的参数,东西都在res里面了
}
...
bool service_c::files(acl::HttpServletRequest & req,
	...
	// 设置响应中的内容字段
	res.setContentType("application/octet-stream");// 多媒体内容类型
	acl::string filename;
	filename.format("attachment;filename=%s", fileid.c_str());
	res.setHeader("content-disposition", filename.c_str());// 设置文件名
	...
	while (remain) { // 还有未下载数据
		...
		free(downdata); // download里面动态分配的,用完要释放
	}
